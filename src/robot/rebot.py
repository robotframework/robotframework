...
"""Rebot -- Robot's Report and Log Generator
...
Inputs to Rebot are XML output files generated by Robot test runs or earlier
Rebot executions. Rebot can be used to generate logs, reports and summary 
reports in HTML format. It can also produce new XML output files which can be 
further processed with Rebot or other tools.
...
Options:
 ...
 -i --include tag *       Select test cases by tag. Similarly as name in --test
                          tag is case and space insensitive and it can also be
                          a simple pattern. To include only tests which have 
                          more than one tag use '&' or 'AND' between tag names.
