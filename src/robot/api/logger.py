...
def enable_background_logging(html=False):
    """By default, writing log message generated by child processes or threads
    is disabled. Call this function to enable the storing of log of messages 
    generated from within a child process or thread. 
    
    You must still call logger.log_background_messages() to write stored log
    messages to the log file.
    """
    write('Background logging enabled', 'DEBUG', html)
    librarylogger.BACKGROUND_LOGGING = True
    

def disable_background_logging(html=False):
    """Calling this function will disable the storing of log messages generated
    by child processes or threads.
    
    If background logging was enabled and is subsequently disabled by calling
    this function, any previously stored log messages created by child 
    processes or threads will be lost.
    """
    write('Background logging disabled', 'DEBUG', html)
    librarylogger.BACKGROUND_LOGGING = False
    
    
def clear_background_logging(html=False):
    """Calling this function will clear an previously stored log messages 
    created by child processes or threads.
    """
    write('Clearing any stored backgound log messages', 'DEBUG', html)
    while librarylogger.MESSAGES:
        librarylogger.MESSAGES.popitem()


def log_background_messages(name=None):
    """Writes messages generated within a child process or thread to the log 
    file.
    
    By default this function writes ALL buffered log messages to the log file. 
    If called with a process/thread name, only those messages generated by that
    specific thread or process are written.
    
    Once the log messages have been written, the relevant thread or process is 
    popped from storage dictionary. 
    """
    if EXECUTION_CONTEXTS.current is not None:
        librarylogger.log_background_messages(name)
    else:
        raise RuntimeError('Cannot call log_background_messages outside of '
                           'robot')
