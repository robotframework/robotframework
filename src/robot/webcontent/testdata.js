window.testdata = function () {
    ...
    function createTest(suite, element, strings, index) {
        ...
        var test = model.Test({
            ...
            status: parseStatus(statusElement, strings, suite.teardownFailed),
            message:  function () {
                var msg = createMessage(statusElement, strings, suite.teardownFailed);
    ...
    function createMessage(statusElement, strings, suiteTeardownFailed) {
        var message = statusElement.length == 4 ? strings.get(statusElement[3]) : '';
        // output generated by rebot has already teardown messages in place
        if (!suiteTeardownFailed || window.output.generator == 'rebot')
            return message;
        if (message)
    ...
    function createSuite(parent, element, strings, index) {
        var statusElement = element[5];
        var parentTeardownFailed = parent && parent.teardownFailed;
        var suite = model.Suite({
            ...
            status: parseStatus(statusElement, strings, parentTeardownFailed),
            message: function () {
