...
class JsonLogger:
    ...
    def end_while(self, item):
        self._end(condition=item.condition,
                  limit=item.limit,
    ...
    def end_try_branch(self, item):
        self._end(patterns=item.patterns,
...
class JsonWriter:
    ...
    def __init__(self, file):
        self.encode = json.JSONEncoder(check_circular=False,
                                       separators=(',', ':'),
                                       default=self._handle_custom).encode
        self.file = file
        self.comma = False
        self.newline = False
