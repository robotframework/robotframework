...
def pause_execution(message='Execution paused. Press OK to continue.'):
    ...
    """
    dialogs_py = importlib.import_module(".dialogs_py", "robot.libraries")
    dialogs_py.MessageDialog(message).show()
...
def execute_manual_step(message, default_error=''):
    ...
    """
    dialogs_py = importlib.import_module(".dialogs_py", "robot.libraries")
    if not _validate_user_input(dialogs_py.PassFailDialog(message)):
...
def get_value_from_user(message, default_value='', hidden=False):
    ...
    """
    dialogs_py = importlib.import_module(".dialogs_py", "robot.libraries")
    return _validate_user_input(dialogs_py.InputDialog(message, default_value,
                                            is_truthy(hidden)))
...
def get_selection_from_user(message, *values, default=None):
    ...
    """
    dialogs_py = importlib.import_module(".dialogs_py", "robot.libraries")
    return _validate_user_input(dialogs_py.SelectionDialog(message, values, default))
...
def get_selections_from_user(message, *values):
    ...
    """
    dialogs_py = importlib.import_module(".dialogs_py", "robot.libraries")
    return _validate_user_input(dialogs_py.MultipleSelectionDialog(message, values))
