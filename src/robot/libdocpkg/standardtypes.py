...
# Paragraphs must not contain soft line breaks: Markdown preserves them in HTML,
# unlike Robot Framework documentation format.
...
""",
    Decimal: """\
Conversion is done using Python's [Decimal](https://docs.python.org/library/decimal.html#decimal.Decimal) class.
...
""",
    date: """\
String timestamps are expected to be in [ISO 8601](https://en.wikipedia.org/wiki/ISO_8601) like date format `YYYY-MM-DD`, where any non-digit character can be used as a separator or separators can be omitted altogether. Possible time components are only allowed if they are zeros.

A special value `NOW` and `TODAY` (case-insensitive) can be used to get the current local `date`. This is new in Robot Framework 7.3.
...
""",
    set: """\
Strings must be Python [set](https://docs.python.org/library/stdtypes.html#set), [list](https://docs.python.org/library/stdtypes.html#list) or [tuple](https://docs.python.org/library/stdtypes.html#tuple) literals. They are converted using the [ast.literal_eval](https://docs.python.org/library/ast.html#ast.literal_eval) function and possible lists and tuples converted further to sets. They can contain any values `ast.literal_eval` supports.

If the argument is a set, it is used without conversion. Lists and other collection objects are converted to sets.

If the type has nested types like `set[int]`, items are converted to those types automatically.

Examples: `{1, 2, 3, 42}`, `set()` (an empty set)

Support to convert nested types is new in Robot Framework 6.0. Support for list and tuple literals is new in Robot Framework 7.4.
""",
    frozenset: """\
Strings must be Python [set](https://docs.python.org/library/stdtypes.html#set), [list](https://docs.python.org/library/stdtypes.html#list) or [tuple](https://docs.python.org/library/stdtypes.html#tuple) literals. They are converted using the [ast.literal_eval](https://docs.python.org/library/ast.html#ast.literal_eval) function and then converted further to `frozenset`. They can contain any values `ast.literal_eval` supports.

If the argument is a frozenset, it is used without conversion. Lists and other collection objects are converted to frozensets.

If the type has nested types like `frozenset[int]`, items are converted to those types automatically.
