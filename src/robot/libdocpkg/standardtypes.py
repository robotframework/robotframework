...
def get_standard_type_doc(typ, doc_format):
    if typ is bytearray:
        typ = bytes
    docs = _std_docs_markdown if doc_format.upper() == "MARKDOWN" else _std_docs_robot
    return docs[typ]
...
""",
    bool: """\
Strings `TRUE`, `YES`, `ON`, `1` and possible localization specific "true strings"
are converted to Boolean `True`, the empty string, strings `FALSE`, `NO`, `OFF`
and `0` and possibly localization specific "false strings" are converted to
Boolean `False`, and the string `NONE` is converted to the Python `None` object.
Other strings and all other values are passed as-is, allowing keywords to handle
them specially if needed.

All string comparisons are case-insensitive.

Examples: `TRUE` (converted to `True`), `off` (converted to `False`), `example`
(used as-is)
""",
    int: """\
Conversion is done using Python's [int] built-in function. Floating point numbers
are accepted only if they can be represented as integers exactly. For example,
`1.0` is accepted but `1.1` is not.

It is possible to use hexadecimal, octal and binary numbers by prefixing values
with `0x`, `0o` and `0b`, respectively.
...
""",
    Decimal: """\
Conversion is done using Python's [Decimal] class.

Spaces and underscores can be used as visual separators for digit grouping purposes.

Examples: `3.14`, `10 000.000 01`

[Decimal]: https://docs.python.org/library/decimal.html#decimal.Decimal
""",
    str: """\
...
""",
    bytes: """\
Strings are converted to bytes so that each Unicode code point below 256 is
directly mapped to a matching byte. Higher code points are not allowed.
Robot Framework's `\\xHH` escape syntax is convenient with bytes having
...
""",
    datetime: """\
Timestamps are expected to be in [ISO 8601] like format `YYYY-MM-DD hh:mm:ss.mmmmmm`.
Any non-digit character can be used as a separator or separators can be omitted
altogether. Additionally, only the date part is mandatory, all possibly missing
...
Examples: `2022-02-09T16:39:43.632269`, `20220209 16:39`, `now`,
...
""",
    date: """\
Timestamps are expected to be in [ISO 8601] like date format `YYYY-MM-DD`. Any
non-digit character can be used as a separator or separators can be omitted
altogether. Possible time components are only allowed if they are zeros.
...
""",
    timedelta: """\
Strings are expected to represent a time interval in one of the time formats
...
See the [User Guide] for more details about the supported time formats.

[User Guide]: https://robotframework.org/robotframework/latest/RobotFrameworkUserGuide.html#time-format
""",
    Path: """\
Strings are converted [Path] objects. On Windows `/` is converted to `\\`
automatically.

Examples: `/tmp/absolute/path`, `relative/path/to/file.ext`, `name.txt`

[Path]: https://docs.python.org/library/pathlib.html
""",
    NoneType: """\
String `NONE` (case-insensitive) and the empty string are converted to the Python
`None` object. Other values cause an error.

Converting the empty string is new in Robot Framework 7.4.
""",
    Sequence: """\
Strings must be Python [list] or [tuple] literals. They are converted to actual
lists or tuples using the [ast.literal_eval] function. They can contain any
...
If the type has nested types like `Sequence[int]`, items are converted to those
types automatically.

Examples: `['one', 'two']`, `(1, 2, 3)`

Support to convert nested types is new in Robot Framework 6.0. Support for tuple
literals is new in Robot Framework 7.4.

[list]: https://docs.python.org/library/stdtypes.html#list
[tuple]: https://docs.python.org/library/stdtypes.html#tuple
[ast.literal_eval]: https://docs.python.org/library/stdtypes.html#ast.literal_eval
""",
    list: """\
Strings must be Python [list] or [tuple] literals. They are converted using the
[ast.literal_eval] function and possible tuples are converted further to lists.
They can contain any values `ast.literal_eval` supports, including lists and
other collections.

If the argument is a list, it is used without conversion. Tuples and other
sequences are converted to lists.

If the type has nested types like `list[int]`, items are converted to those
types automatically.
...
""",
    tuple: """\
Strings must be Python [tuple] or [list] literals. They are converted using the
[ast.literal_eval] function and possible lists are converted further to tuples.
They can contain any values `ast.literal_eval` supports, including lists and
other collections.

If the argument is a tuple, it is used without conversion. Lists and other
sequences are converted to tuples.

If the type has nested types like `tuple[str, int]`, items are converted to
those types automatically.

Examples: `('one', 'two')`, `[('one', 1), ('two', 2)]`

Support to convert nested types is new in Robot Framework 6.0. Support for list
literals is new in Robot Framework 7.4.
...
""",
    Mapping: """\
Strings must be Python [dictionary] literals. They are converted to actual
dictionaries using the [ast.literal_eval] function. They can contain any values
`ast.literal_eval` supports, including dictionaries and other collections.

Any mapping is accepted without conversion. An exception is that if the type is
`MutableMapping`, immutable values are converted to `dict`.

If the type has nested types like `Mapping[str, int]`, items are converted to
those types automatically. This in new in Robot Framework 6.0.

Examples: `{'a': 1, 'b': 2, 'c': 3}`, `{'key': 1, 'nested': {'key': 2}}`

[dictionary]: https://docs.python.org/library/stdtypes.html#dict
[ast.literal_eval]: https://docs.python.org/library/stdtypes.html#ast.literal_eval
""",
    dict: """\
Strings must be Python [dictionary] literals. They are converted to actual
...
If the type has nested types like `dict[str, int]`, items are converted to those
types automatically. This in new in Robot Framework 6.0.
...
""",
    set: """\
Strings must be Python [set], [list] or [tuple] literals. They are converted
using the [ast.literal_eval] function and possible lists and tuples converted
further to sets. They can contain any values `ast.literal_eval` supports.

If the argument is a set, it is used without conversion. Lists and other
collection objects are converted to sets.

If the type has nested types like `set[int]`, items are converted to those types
automatically.

Examples: `{1, 2, 3, 42}`, `set()` (an empty set)

Support to convert nested types is new in Robot Framework 6.0. Support for list
and tuple literals is new in Robot Framework 7.4.
...
""",
    frozenset: """\
Strings must be Python [set], [list] or [tuple] literals. They are converted
using the [ast.literal_eval] function and then converted further to `frozenset`.
They can contain any values `ast.literal_eval` supports.

If the argument is a frozenset, it is used without conversion. Lists and other
collection objects are converted to frozensets.

If the type has nested types like `frozenset[int]`, items are converted to those
types automatically.

Examples: `{1, 2, 3, 42}`, `frozenset()` (an empty frozenset)
...
""",
    Literal: """\
Only specified values are accepted. Values can be strings, integers, bytes,
Booleans, enums and None, and used arguments are converted using the value type
specific conversion logic.

String matching is case, space, underscore and hyphen insensitive, but exact
matches have precedence over normalized matches.
""",
    Secret: """\
Encapsulates secret values to avoid them being shown in Robot Framework logs.

The value is required to be a [robot.api.types.Secret] object. These objects
encapsulate confidential values so that they are not exposed in log files.
How to create them is explained in the [User Guide].

New in Robot Framework 7.4.

[robot.api.types.Secret]: https://robot-framework.readthedocs.io/en/master/autodoc/robot.utils.html#robot.utils.secret.Secret
[User Guide]: https://robotframework.org/robotframework/latest/RobotFrameworkUserGuide.html#secret-variables
""",
}
...
built-in function. Floating point numbers are accepted only if they can be
represented as integers exactly. For example, ``1.0`` is accepted and ``1.1`` is not.
