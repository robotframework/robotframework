...
class ReturnValueResolver:
    ...
    def _split_assignment(self, assignment, multi=False):
        match: VariableMatch = search_variable(assignment)
        if ': ' in match.base:
            base, type_ = match.base.rsplit(': ', 1)
            if not multi and match.identifier == '@':
                type_ = f'list[{type_}]'
            elif not multi and match.identifier == '&':
                k_type, v_type = type_.split('=') if '=' in type_ else ('Any', type_)
                type_ = f'dict[{k_type}, {v_type}]'
            name = f'{match.identifier}{{{base}}}'
        else:
            name = match.name
            type_ = None
        return name, type_, match.items
...
class MultiReturnValueResolver(ReturnValueResolver):
    ...
    def __init__(self, assignments):
        ...
        for assign in assignments:
            name, type_, items = self._split_assignment(assign, multi=True)
