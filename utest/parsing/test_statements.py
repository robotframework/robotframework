...
from robot.parsing.model.statements import (
    Arguments, Break, Comment, Continue, DefaultTags, Documentation, ElseHeader,
    ElseIfHeader, EmptyLine, End, ExceptHeader, FinallyHeader, ForHeader, GroupHeader,
    IfHeader, InlineIfHeader, KeywordCall, KeywordName, KeywordTags, LibraryImport,
    Metadata, ResourceImport, ReturnSetting, ReturnStatement, SectionHeader, Setup,
    Statement, SuiteSetup, SuiteTeardown, Tags, Teardown, Template, TemplateArguments,
    TestCaseName, TestSetup, TestTags, TestTeardown, TestTemplate, TestTimeout, Timeout,
    TryHeader, Var, Variable, VariablesImport, WhileHeader
)
...
class TestCreateStatementsFromParams(unittest.TestCase):
    ...
    def test_Variable(self):
        ...
        for name in ["${var}", "@{var}", "&{var}"]:
            ...
            assert_created_statement(
                tokens, Variable, name=name, value=["first", "second", "third"]
            )

    def test_TestCaseName(self):
        tokens = [
            Token(Token.TESTCASE_NAME, "Example test case name"),
            Token(Token.EOL, "\n"),
        ]
    ...
    def test_KeywordName(self):
        tokens = [
            Token(Token.KEYWORD_NAME, "Keyword Name With ${embedded} Var"),
            Token(Token.EOL, "\n"),
        ]
        assert_created_statement(
            tokens, KeywordName, name="Keyword Name With ${embedded} Var"
        )
    ...
    def test_Teardown(self):
        ...
        assert_created_statement(
            tokens, Teardown, name="Teardown Keyword", args=["${arg1}"]
        )
    ...
    def test_LibraryImport(self):
        ...
        assert_created_statement(
            tokens, LibraryImport, name="library_name.py", alias="anothername"
        )
    ...
    def test_ResourceImport(self):
        ...
        assert_created_statement(
            tokens, ResourceImport, name="path${/}to${/}resource.robot"
        )
    ...
    def test_VariablesImport(self):
        ...
        assert_created_statement(
            tokens, VariablesImport, name="variables.py", args=["arg1", "2"]
        )
    ...
    def test_Documentation(self):
        ...
        doc = assert_created_statement(
            tokens, Documentation, value="Example documentation"
        )
    ...
    def test_Metadata(self):
        ...
        assert_created_statement(
            tokens, Metadata, name="Key", value="First line\nSecond line"
        )
    ...
    def test_DefaultTags(self):
        ...
        assert_created_statement(
            tokens, DefaultTags, values=["some tag", "another_tag"]
        )
    ...
    def test_InlineIfHeader_with_assign(self):
        ...
        assert_created_statement(
            tokens, InlineIfHeader, condition="$x > 0", assign=["${y}"]
        )
    ...
    def test_ElseIfHeader(self):
        ...
        assert_created_statement(
            tokens, ElseIfHeader, condition="${var} not in [@{list}]"
        )
    ...
    def test_ExceptHeader(self):
        ...
        assert_created_statement(
            tokens, ExceptHeader, patterns=["one", "two"], assign="${var}"
        )
        ...
        assert_created_statement(
            tokens, ExceptHeader, patterns=["Example: *"], type="glob"
        )
    ...
    def test_GroupHeader(self):
        ...
        assert_created_statement(tokens, GroupHeader, name="name")
        # GROUP
        tokens = [
            Token(Token.SEPARATOR, "    "),
            Token(Token.GROUP),
        ...
        assert_created_statement(tokens, GroupHeader, name="")
