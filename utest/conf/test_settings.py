...
class TestRobotAndRebotSettings(unittest.TestCase):
    ...
    def test_output_files_with_timestamps(self):
        for name in (
            ...
            for value in "test", Path("test"):
                path = RobotSettings(
                    {base.lower(): value, "timestampoutputs": True},
                )[base]
                assert_true(isinstance(path, Path))
                assert_equal(
                    f"test-<timestamp>.{ext}",
            ...
            for value in "None", "NONE", None:
                for timestamp_outputs in True, False:
                    settings = RobotSettings(
                        {name.lower(): value, "timestampoutputs": timestamp_outputs}
                    )
    ...
    def test_output_dir(self):
        for value in ".", Path("."), Path(".").absolute():
            assert_equal(
                RobotSettings({"outputdir": value}).output_directory,
                Path(".").absolute(),
            )
