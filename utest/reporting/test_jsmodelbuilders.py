...
from robot.model import BodyItem, Statistics
from robot.reporting.jsmodelbuilders import (
    BodyItemBuilder, ErrorsBuilder, JsBuildingContext, MessageBuilder,
    StatisticsBuilder, SuiteBuilder, TestBuilder
)
from robot.reporting.stringcache import StringIndex
from robot.result import For, ForIteration, Keyword, Message, TestCase, TestSuite
...
class TestBuildTestSuite(unittest.TestCase):
    ...
    def test_relative_source(self):
        self._verify_suite(
            TestSuite(source="non-existing"),
            name="Non-Existing",
            source="non-existing",
        )
        source = CURDIR / "test_jsmodelbuilders.py"
        self._verify_suite(
            TestSuite(name="x", source=source),
            name="x",
            source=str(source),
            relsource=str(source.name),
        )

    def test_suite_html_formatting(self):
        self._verify_suite(
            TestSuite(name="*xxx*", doc="*bld* <&>", metadata={"*x*": "*b*", "<": ">"}),
            name="*xxx*",
            doc="<b>bld</b> &lt;&amp;&gt;",
    ...
    def test_name_escaping(self):
        ...
        self._verify_body_item(
            kw, 0, "quote:&quot;", "and *url* https://url.com", '<b>"Doc"</b>'
        )
        test = TestCase('quote:" and *url* https://url.com', '*"Doc"*')
        self._verify_test(
            test, "quote:&quot; and *url* https://url.com", '<b>"Doc"</b>'
        )
    ...
    def test_keyword_with_values(self):
        kw = Keyword(
            ...
            "FAIL",
            "message",
            "2011-12-04 19:42:42.000",
            "2011-12-04 19:42:42.042",
        )
        self._verify_body_item(
            kw,
            1,
    ...
    def test_nested_structure(self):
        ...
        f = self._verify_body_item(
            suite.tests[0].body[0], type=3, name="${x}    IN    1    2", body=(i, m)
        )
    ...
    def test_timestamps(self):
        ...
        suite.setup.body.create_message(
            level="DEBUG", timestamp="2011-12-05 00:33:33.344"
        )
    ...
    def test_for(self):
        ...
        f2 = self._verify_body_item(
            test.body[1], type=3, name="${x}    IN ENUMERATE    a    b    start=1"
        )
    ...
    def test_return(self):
        ...
        self._verify_body_item(
            Keyword().body.create_return(("only one value",)),
            type=8,
            args="only one value",
        )
        self._verify_body_item(
            Keyword().body.create_return(("more", "than", "one")),
            type=8,
            args="more    than    one",
        )
    ...
    def test_var(self):
        ...
        v2 = self._verify_body_item(
            test.body[1], type=9, name="${y}    x    y    separator=    scope=test"
        )
        v3 = self._verify_body_item(
            test.body[2], type=9, name="@{z}    x    y    scope=SUITE"
        )
...
class TestSplitting(unittest.TestCase):
    ...
    def test_test_keywords(self):
        ...
        assert_equal(
            [strings for _, strings in context.split_results],
            [("*", "*t1-k1", "*t1-k1-k1", "*t1-k2"), ("*", "*t2-k1")],
        )
        assert_equal(
            [self._to_list(remap(*res)) for res in context.split_results],
            expected_split,
        )
    ...
    def _to_list(self, model):
        return [
            self._to_list(item) if isinstance(item, tuple) else item for item in model
        ]
...
class TestBuildStatistics(unittest.TestCase):
    ...
    def test_suite_stats(self):
        ...
        self._verify_stat(
            sub1, 1, 1, 1, "root.sub1", "00:00:10", name="sub1", id="s1-s1"
        )
        self._verify_stat(
            sub2, 1, 1, 0, "root.sub2", "00:00:30", name="sub2", id="s1-s2"
        )
    ...
    def _get_suite(self):
        ts = lambda s, ms=0: f"2012-08-16 16:09:{s:02}.{ms:03}"
        ...
        sub1.tests = [
            TestCase(
                tags=["t1", "t2"], status="PASS", start_time=ts(0), end_time=ts(1, 500)
            ),
            TestCase(
                tags=["t1", "t3"], status="FAIL", start_time=ts(2), end_time=ts(3, 499)
            ),
            TestCase(
                tags=["t3"], status="SKIP", start_time=ts(3, 560), end_time=ts(3, 560)
            ),
        ]
        sub2.tests = [
            TestCase(
                tags=["t1", "t2"], status="PASS", start_time=ts(10), end_time=ts(30)
            )
        ]
        sub2.suites.create(name="below suite stat level").tests.create(
            tags=["t1"], status="FAIL", start_time=ts(30), end_time=ts(40)
        )
        return suite

    def _verify_stat(self, stat, pass_, fail, skip, label, elapsed, **extra):
        stats = {
            "pass": pass_,
            "fail": fail,
            "skip": skip,
...
class TestBuildErrors(unittest.TestCase):
    ...
    def setUp(self):
        msgs = [
            Message("Error", "ERROR", timestamp="2011-12-06 14:33:00.000"),
