...
class TestCustomMetadataModifiers(unittest.TestCase):
        ...
        class MetadataAnalyzer(SuiteVisitor):
            ...
            def visit_test(self, test):
                result = {
                    ...
                    "metadata_count": len(dict(test.custom_metadata)) if test.has_custom_metadata else 0,
                    "metadata_keys": list(test.custom_metadata.keys()) if test.has_custom_metadata else []
                }
    ...
    def test_listener_integration_with_execution_model(self):
        ...
        expected_metadata = {
            "Test With Metadata": {"Owner": "Alice", "Priority": "High", "Component": "UI"},
            "Test Without Metadata": {},
            "Another Test With Metadata": {"Owner": "Bob", "Environment": "Staging", "Bug-ID": "BUG-123"}
        }
            ...
            def visit_test(self, test):
                try:
                    ...
                    if not hasattr(test, 'custom_metadata'):
                        self.errors_encountered.append(f"No custom_metadata attr on {test.name}")
                        return
