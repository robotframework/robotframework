...
class TestCustomMetadataBuilder(unittest.TestCase):
    ...
    def test_custom_metadata_in_keywords_and_mixed_settings(self):
        ...
        suite = self._build_from_string(
            data, custom_metadata=["Owner", "Priority", "API"]
        )
    ...
    def test_custom_metadata_special_values_and_continuation(self):
        ...
        expected_desc = (
            "This is a very long description that continues on the next line"
        )
    ...
    def test_file_settings_and_integration_scenarios(self):
        ...
        settings_allowed = SuiteFileSettings(
            Languages(), allowed_custom_metadata=allowed
        )
        ...
        settings_case = SuiteFileSettings(
            Languages(), allowed_custom_metadata=["Owner", "priority"]
        )
        assert_true(settings_case.is_custom_metadata_allowed("Owner"))
        assert_true(settings_case.is_custom_metadata_allowed("priority"))
        assert_false(
            settings_case.is_custom_metadata_allowed("owner")
        )  # Different case
        assert_false(
            settings_case.is_custom_metadata_allowed("Priority")
        )  # Different case
