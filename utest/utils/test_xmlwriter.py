...
class TestXmlWriter(unittest.TestCase):
    ...
    def test_write_many_elements(self):
        ...
        self._verify_node(
            root.find("child1/leaf1.1"), "leaf1.1", "leaf content", {"type": "kw"}
        )
    ...
    def _verify_node(self, node, name, text=None, attrs=None):
        if node is None:
            node = self._get_root()
        assert_equal(node.tag, name)
        if text is not None:
            assert_equal(node.text, text)
        assert_equal(node.attrib, attrs or {})
