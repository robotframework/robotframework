...
class TestDecodeOutput(unittest.TestCase):
    ...
    def test_return_unicode_as_is_by_default(self):
        assert_equals(console_decode(UNICODE), UNICODE)

    if not IRONPYTHON:

        def test_decode(self):
            assert_equals(console_decode(ENCODED), UNICODE)
        ...
        def test_force_decoding(self):
            assert_equals(console_decode(ENCODED, force=True), UNICODE)

        def test_bytes_are_decoded(self):
            assert_equals(console_decode(bytes(ENCODED)), UNICODE)
