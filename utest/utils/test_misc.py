...
class TestTestOrTask(unittest.TestCase):
    ...
    def test_multiple_matches(self):
        assert_equal(
            test_or_task("Contains {test}, {TEST} and {TesT}", False),
            "Contains test, TEST and TesT",
        )
        assert_equal(
            test_or_task("Contains {test}, {TEST} and {TesT}", True),
...
"""
        assert_raises_with_msg(
            TypeError, "Setters are not supported.", exec, code, globals()
        )
        assert_raises_with_msg(
            TypeError,
