...
from robot.utils.robottime import (
    _get_timetuple, elapsed_time_to_string, format_time, get_elapsed_time, get_time,
    get_timestamp, parse_time, parse_timestamp, secs_to_timestr, timestamp_to_secs,
    timestr_to_secs
)
...
class TestTime(unittest.TestCase):
    ...
    def test_timestr_to_secs_with_time_string(self):
        for inp, exp in [
            ...
            ("1 w", 7 * 24 * 60 * 60),
            ("2w", 2 * 7 * 24 * 60 * 60),
            ("1w 0d 0h 0m 0s 0ms", 7 * 24 * 60 * 60),
            ("2w 0d 0h 0m 0s 0ms", 2 * 7 * 24 * 60 * 60),
            ("1 week 1 day 1 hour 1 minute 1 second", 8 * 24 * 60 * 60 + 3661),
    ...
    def test_get_timestamp(self):
        for seps, pattern in [
            ...
            assert_not_none(
                re.search(pattern, ts),
                f"'{ts}' didn't match '{pattern}'",
                values=False,
            )
    ...
    def test_timestamp_to_secs(self):
        with warnings.catch_warnings(record=True):
            assert_equal(
                timestamp_to_secs("20070920 16:15:14.123"),
                EXAMPLE_TIME + 0.123,
            )
            assert_equal(
                timestamp_to_secs("20070920T16:15:14.123"),
                EXAMPLE_TIME + 0.123,
            )
            assert_equal(
                timestamp_to_secs("2007-09-20#16x15x14M123", ("-", "#", "x", "M")),
                EXAMPLE_TIME + 0.123,
            )
            assert_equal(
                timestamp_to_secs("20070920 16:15:14.123"),
    ...
    def test_elapsed_time_to_string(self):
        for elapsed, expected in [
            ...
            (360000 + 36000 + 3600 + 660 + 11.111, "111:11:11.111"),
        ]:
            assert_equal(
    ...
    def test_elapsed_time_to_string_without_millis(self):
        for elapsed, expected in [
            ...
            assert_equal(
                elapsed_time_to_string(elapsed, include_millis=False, seconds=True),
                expected,
                elapsed,
            )
    ...
    def test_parse_time_and_get_time_must_round_seconds_down(self):
        # Rounding to the closest second, instead of rounding down, could give
        # times that are greater than e.g. timestamps of files created later.
    ...
    def _verify_parse_time_and_get_time_rounding(self):
        ...
        if secs() == start_secs:
            assert_equal(gt_result, format(start_secs, "02"))
