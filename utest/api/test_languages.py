...
from robot.utils.asserts import (
    assert_equal, assert_not_equal, assert_raises, assert_raises_with_msg
)
...
class TestLanguage(unittest.TestCase):
        ...
        for cls in Language.__subclasses__():
            for attr in dir(cls):
                if not hasattr(Language, attr):
                    raise AssertionError(
                        f"Language class '{cls}' has attribute "
    ...
    def test_bdd_prefixes(self):
        ...
        assert_equal(
            X().bdd_prefixes,
            {"List", "is", "default", "but", "any", "iterable", "works"},
        )
...
class TestLanguageFromName(unittest.TestCase):
    ...
    def test_no_match(self):
        assert_raises_with_msg(
            ValueError,
            "No language with name 'no match' found.",
            Language.from_name,
            "no match",
        )
...
class TestLanguages(unittest.TestCase):
    ...
    def test_init_with_custom_language(self):
        ...
        for lang in (
            ...
            assert_equal(
                [("Orcish Loud", "or-CLOU"), ("Orcish Quiet", "or-CQUI")],
