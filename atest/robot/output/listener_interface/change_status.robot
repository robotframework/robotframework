...
Fail to pass
    ...
    Check Log Message     ${tc[0].msgs[0]}    Pass me!    level=FAIL
...
Pass to fail
    ...
    Check Log Message     ${tc[0].msgs[0]}    Fail me!    level=INFO
...
Skip to fail
    ...
    Check Log Message     ${tc[0].msgs[0]}    Fail me!    level=SKIP
    Check Keyword Data    ${tc[1]}    BuiltIn.Log     args=I'm not run.    status=NOT RUN    message=

Fail to skip
    ${tc} =    Check Test Case    ${TEST NAME}
    Check Keyword Data    ${tc[0]}    BuiltIn.Fail    args=Skip me!        status=SKIP       message=Skipping!
    Check Log Message     ${tc[0].msgs[0]}    Skip me!    level=FAIL
    Check Keyword Data    ${tc[1]}    BuiltIn.Log     args=I'm not run.    status=NOT RUN    message=
