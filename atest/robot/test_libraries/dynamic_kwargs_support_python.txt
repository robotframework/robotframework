*** Settings ***
Suite Setup     Run Tests  ${EMPTY}  test_libraries/dynamic_kwargs_support_python.txt
...
Documentation and Argument Boundaries Work With Kwargs
    Check test case and its keywords  Kwargs  key:value
...
Check test case and its keywords
    ...
    @{argstrings}=  Evaluate  enumerate(${argstrings})
    @{argstrings}=  Combine Lists  @{argstrings}
    :FOR  ${id}  ${argstr}  IN  @{argstrings}
    \   Check Log Message  ${tc.kws[${id}].msgs[0]}
        ...  Executed keyword ${keyword} with arguments ${argstr}
