...
*** Variables ***
${PATH}           ${{os.path.normpath($DATADIR+'/test_libraries')}}

*** Test Cases ***
Import Library Normally Before Importing With Name In Another Suite
    ${tc} =    Check Test Case    ${TEST NAME}
    Check Keyword Data    ${tc.kws[0]}    OperatingSystem.Should Exist    args=${PATH}
...
No Arguments
    ${tc} =    Check Test Case    ${TEST NAME}
    Check Keyword Data    ${tc.kws[0]}    OS.Directory Should Exist    args=${PATH}
    Check Keyword Data    ${tc.kws[1]}    OS.Should Exist    args=${PATH}
...
Import Library Keyword
    ${tc} =    Check Test Case    ${TEST NAME}
    Check Keyword Data    ${tc.kws[1]}    MyOS.Directory Should Exist    args=${PATH}
