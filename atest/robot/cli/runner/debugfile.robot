...
Debugfile
    ...
    ...      *\tMainThread\tstd_thread\tinterpreter_0\t${TIMESTAMP} - INFO - +-- START KEYWORD: BuiltIn.Log ? Test 1 ?
    ...      *\tMainThread\tstd_thread\tinterpreter_0\t${TIMESTAMP} - INFO - Test 1
    ...      *\tMainThread\tstd_thread\tinterpreter_0\t${TIMESTAMP} - INFO - +-- END KEYWORD: BuiltIn.Log
    Debug file should contain    ${content}
    ...      *\tMainThread\tstd_thread\tinterpreter_0\t${TIMESTAMP} - INFO - +-- START KEYWORD: BuiltIn.Log ? Logging with debug level | DEBUG ?
    ...      *\tMainThread\tstd_thread\tinterpreter_0\t${TIMESTAMP} - DEBUG - Logging with debug level
    ...      *\tMainThread\tstd_thread\tinterpreter_0\t${TIMESTAMP} - INFO - +-- END KEYWORD: BuiltIn.Log
...
Debugfile timestamps are accurate
    ...
    ${content} =     Get file     ${CLI OUTDIR}/debug.txt
    Debug file should contain    ${content}
    ...    *\tMainThread\tstd_thread\tinterpreter_0\t${tc[0, 0].timestamp} - INFO - Known timestamp
    ...    *\tMainThread\tstd_thread\tinterpreter_0\t${tc[0, 1].timestamp} - INFO - <b>Current</b>
