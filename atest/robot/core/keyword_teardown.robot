...
Teardown in keyword with embedded arguments
    ...
    Check Log Message  ${tc.kws[0].kws[1].msgs[0]}  In Teardown of UK with Embedded Arguments
    Check Log Message  ${tc.kws[1].kws[0].msgs[0]}  Expected Failure in UK with Embedded Arguments  FAIL
    Check Log Message  ${tc.kws[1].kws[1].msgs[0]}  In Teardown of Failing UK with Embedded Arguments
...
Multiple Failures in Keyword Teardown
    ${tc}=  Check Test Case  ${TESTNAME}
    Check Log Message  ${tc.kws[0].kws[1].kws[0].msgs[0]}  Failure in Teardown  FAIL
    Check Log Message  ${tc.kws[0].kws[1].kws[1].kws[0].msgs[0]}  Expected Failure!  FAIL
    Check Log Message  ${tc.kws[0].kws[1].kws[1].kws[1].msgs[0]}  Executed if in nested Teardown
    Check Log Message  ${tc.kws[0].kws[1].kws[2].msgs[0]}  Third failure in Teardown  FAIL
...
Nested Keyword Teardown Failures
    ${tc}=  Check Test Case  ${TESTNAME}
    Check Log Message  ${tc.kws[0].kws[0].kws[1].msgs[0]}  Failing in UK Teardown  FAIL
    Check Log Message  ${tc.kws[0].kws[1].msgs[0]}  Failing in outer UK Teardown  FAIL
