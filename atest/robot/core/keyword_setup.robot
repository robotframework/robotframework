...
Passing setup
    ...
    Check Log Message     ${tc[0].setup.msgs[0]}         Hello, setup!

Failing setup
    ${tc} =    Check Test Case    ${TESTNAME}
    Check Log Message    ${tc[0].setup.msgs[0]}      Hello, setup!          FAIL
...
Failing setup and teardown
    ...
    Check Log Message    ${tc[0].setup.msgs[0]}       Hello, setup!          FAIL
    Should Be Equal      ${tc[0, 0].status}           NOT RUN
    Check Log Message    ${tc[0].teardown.msgs[0]}    Hello, teardown!       FAIL

Continue-on-failure mode is not enabled in setup
    ${tc} =    Check Test Case    ${TESTNAME}
    Check Log Message    ${tc.setup.setup[0].msgs[0]}    Hello, setup!          INFO
    Check Log Message    ${tc.setup.setup[1].msgs[0]}    Hello again, setup!    FAIL
