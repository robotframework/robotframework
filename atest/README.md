...
- [Dependencies](#dependencies)
  - [Execution side dependencies](#execution-side-dependencies)
  - [Test data side dependencies](#test-data-side-dependencies)
    - [Needed Python modules](#needed-python-modules)
    - [Screenshot module or tool](#screenshot-module-or-tool)
- [Schema validation](#schema-validation)
  - [output.xml schema](#outputxml-schema)
  - [Libdoc XML and JSON spec schemas](#libdoc-xml-and-json-spec-schemas)
- [Telnet tests](#telnet-tests)
...
**`run.py`**
    A script for executing acceptance tests. See [Running acceptance tests](#running-acceptance-tests)
    for further instructions.

**`robot/`**
    Contains the actual acceptance tests. See the [Test data](#test-data) section for details.
...
**`testdata/`**
    ...
    the [Test data](#test-data) section for details.

**`testresources/`**
    Contains resources needed by test cases in the `testdata` folder.
    Some of these resources are also used by [unit tests](../utest/README.rst).
...
Robot Framework's acceptance tests are executed using the [`run.py`](run.py)
...
with `no-ci` are executed. See the [Test tags](#test-tags) section below for more
...
`--schema-validation` can be used to enable [schema validation](#schema-validation) for all output.xml
files.
...
([atest/robot](robot) directory) and the test data side
([atest/testdata](testdata) directory). The test data side contains test
...
Selecting tests based on the platform is done automatically by the [`run.py`](run.py)
script, but additional selection can be done by the user, for example, to
avoid running tests with [dependencies](#dependencies) that are not met.

- **manual** — Require manual interaction from user. Used with Dialogs library tests.

- **telnet** — Require a Telnet server with test account running on localhost. See [Telnet tests](#telnet-tests) for details.

- **no-ci** — Tests which are not executed at continuous integration. Contains all tests tagged with `manual` or `telnet`.

- **require-yaml, require-lxml, require-screenshot** — Require specified Python module or some other external tool to be installed. Exclude like `--exclude require-lxml` if [dependencies](#dependencies) are not met.

- **require-windows, require-py3.8, ...** — Tests that require certain operating system or Python interpreter. Excluded automatically outside these platforms.

- **no-windows, no-osx, ...** — Tests to be excluded on certain operating systems or Python interpreters. Excluded automatically on these platforms.
...
run all tests successfully. See [Test tags](#test-tags) for instructions how to avoid
running certain tests if all preconditions are not met.

### Execution side dependencies

The execution side has some dependencies listed in [`requirements-run.txt`](requirements-run.txt)
...
- [docutils](http://docutils.sourceforge.net/) is needed with tests related
  to parsing test data in reStructuredText format and with Libdoc tests
  for documentation in reST format.
- [Pygments](http://pygments.org/) is needed by Libdoc tests for syntax
  highlighting.
- [PyYAML](http://pyyaml.org/) is required with tests related to YAML
  variable files.
- [Pillow](https://pypi.org/project/Pillow/) for taking screenshots on Windows.
- [lxml](http://lxml.de/) is needed with XML library tests.

It is possible to install the above modules using `pip` individually, but
it is easiest to use the provided [`requirements.txt`](requirements.txt) file that installs
...
take screenshots. The above instructions already covered installing [Pillow](https://pypi.org/project/Pillow/)
...
[Screenshot library documentation](http://robotframework.org/robotframework/latest/libraries/Screenshot.html).

## Schema validation

### output.xml schema

Created output.xml has a [schema](../doc/schema) that can be tested as part of
acceptance tests. The schema is always used to validate selected outputs (e.g. in
[`robot/rebot/compatibility.robot`](robot/rebot/compatibility.robot)), but validating all outputs would slow down
...
[schemas](../doc/schema). All generated Libdoc specs are validated automatically
in Libdoc tests.

## Telnet tests

Running telnet tests requires some extra setup. Instructions how to run them
can be found from [`testdata/standard_libraries/telnet/README.rst`](testdata/standard_libraries/telnet/README.rst).
