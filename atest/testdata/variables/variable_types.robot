...
Assignment
    ...
    Varargs    1    2    3
    Kwargs    a=1    b=2.3
    Kwargs type and value   1=2.3    4=5.6
...
Keyword
    ...
    Variable should not exist    ${arg: int|float}

Varargs
    [Arguments]    @{args: int}
    Should be equal    ${args}    [1, 2, 3]    type=list[int]
    Variable should not exist    @{args: int}

Kwargs
    [Arguments]    &{args: int|float}
    Should be equal    ${args}    {"a":1, "b":2.3}    type=dict[str, int|float]
    Variable should not exist    &{args: int|float}

Kwargs type and value
    [Arguments]    &{args: int=float}
    Should be equal    ${args}    {1:2.3, 4:5.6}    type=dict[int, float]


Combination of all args
    [Arguments]    ${arg: float}    @{args: int}    &{kwargs: str=int}
    ...
    Variable should not exist    ${arg: float}
    Variable should not exist    @{args: int}
    Variable should not exist    &{kwargs: str=int}
