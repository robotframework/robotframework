...
Multiple unexpected keyword argument with inequality
    [Documentation]    FAIL Keyword 'Kw Only Arg' got unexpected named arguments '<' and 'ooops'.
    Kw Only Arg    kwo=value    <=ooops    ooops=invalid

Multiple unexpected keyword argument with escaped inequality
    [Documentation]    FAIL Keyword 'Kw Only Arg' got positional argument after named arguments.
    Kw Only Arg    kwo=value    <\=ooops    ooops=invalid
...
With two named arguments but with a error
    [Documentation]  FAIL    Keyword 'Kw Two Args With Varargs' got positional argument after named arguments.
...
Kw Two Args With Varargs
    [Arguments]    @{varargs}    ${kwo}=default   ${another}=another
    ${result} =    Catenate    SEPARATOR=-    @{varargs}    ${kwo}    ${another}
    [Return]    ${result}
